...
| کد | موضوع | اولویت | نوع تغییر |
| --- | --- | --- | --- |
| B1 | قرارداد چندکلاینته پایدار روی همان ABR | P0 | مستند + در صورت نیاز client غیر Vue / ضمانت سازگاری |
| B2 | Auth قابل استفاده در embed / iframe (نه فقط کوکی + hostname) | P0 | additive: bearer + refresh + نقش از API |
| B3 | نوتیف وقتی تب مرورگر موبایل در پس‌زمینه است | P0 | رفتار reconnect / Notification؛ کانال رویداد جدید نه |
| B4 | Authorization سمت سرور برای list / filter / convey | P0 | تأیید؛ اگر نیست، enforce |
| B5 | معناشناسی availability وقتی تب در پس‌زمینه / منجمد است | P1 | تعریف رفتار + در صورت نیاز hook قطع اتصال |
| B6 | آپلود فایل مناسب مرورگر موبایل روی همان REST path | P1 | multipart + محدودیت رسمی + HEIC |
| B7 | نام و payload رویداد یکدست برای Chat و UI Ticket | P1 | یکسان‌سازی نام؛ موجودیت جدید نه |
| B8 | ماتریس محیط / کشف host برای widget و flavorها | P1 | جدول رسمی؛ endpoint کشف اختیاری |
